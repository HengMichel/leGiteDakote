    ...
    <form method="post">
        <div class="row">
            <div class="form-group link-warning fw-medium d-flex justify-content-md-center mt-5">
                <input type="text" name="id_room" value="<?= $bookings->getRoom_id() ?>" <?= $mode == "suppression" ? "disabled" : "" ?> hidden>
                <input type="text" name="price" value="<?= $bookings->getPrice() ?>" <?= $mode == "suppression" ? "disabled" : "" ?> hidden>
        ...
        <button type="submit"  class="btn mt-5 mb-5 bg-success link-warning border-warning border-4 fw-medium m-auto" name="book"><?= $mode == "suppression" ? "Confirmer" : "Enregistrer" ?></button><a href="<?= addLink("player") ?>" class="btn btn-success mt-5 mb-5 link-light fw-medium">Annuler</a>
    </form>
</div>
